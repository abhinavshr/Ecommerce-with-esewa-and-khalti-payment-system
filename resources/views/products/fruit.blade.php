...
<body>
    ...
    <div class="sidenav-container" style="display: flex;">
        <div class="sidenav">
            <div class="container">
                <form action=" {{ route('user.fruit.search') }}  " method="GET">
        ...
        <div class="right">
            <h1 class="heading">Fruit</h1>
            <div class="sorting" style="float: right;">
                <form action="{{ route('user.fruit') }}" method="GET">
                    <select name="sort" id="sort" onchange="this.form.submit()" style="float: right;">
                                        ...
                                        <p>
                                            ...
                                            @if ($product->discounted_price > 0)
                                                <span style="text-decoration: line-through; color: red;">Rs.
                                                    {{ $product->product_price }} Per Kg
                                                </span>
                                                <span style="color: green;">Rs.
                                                    {{ $product->product_price - ($product->product_price * $product->discounted_price) / 100 }}
                                                    Per Kg ({{ $product->discounted_price }}% off)
                                                </span>
                                            @else
                                                Rs. {{ $product->product_price }} Per Kg
                                            @endif
                                        </p>
                                        <p>Quantity: {{ $product->product_quantity }} Kg</p>
